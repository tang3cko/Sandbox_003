...
public partial class TitleScreen : Control
{
    ...
    private void BuildUI()
    {
        ...
        bg.Color = new Color(0.02f, 0.03f, 0.05f, 1);
        ...
        var title = CreateLabel("SWARM SURVIVOR", 80);
        title.HorizontalAlignment = HorizontalAlignment.Center;
        vbox.AddChild(title);

        var subtitle = CreateLabel("Phase 6: GDShader / MultiMesh / CompositorEffect", 36);
        subtitle.HorizontalAlignment = HorizontalAlignment.Center;
        subtitle.AddThemeColorOverride("font_color", new Color(0.6f, 0.6f, 0.7f));
        vbox.AddChild(subtitle);

        var controls = CreateLabel("WASD: Move  |  ESC: Pause", 28);
        controls.HorizontalAlignment = HorizontalAlignment.Center;
        controls.AddThemeColorOverride("font_color", new Color(0.5f, 0.5f, 0.55f));
        vbox.AddChild(controls);

        var desc = CreateLabel("Weapons fire automatically. Collect XP gems to level up.", 28);
